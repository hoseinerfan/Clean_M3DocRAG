...
The author supplied successful accounting and the validation result for BGE
job 15911612 on gpu009: 2,441 questions, four pages each, EM 37.85 / F1 43.89.
The baseline guide now includes these QA columns in its manual LaTeX table.
...
Job 15911612 completed (0:0, 02:26:55, gpu009). The author-provided final JSON
and `BGE_READER_RESULT` both report 2441 questions, four pages each, EM
37.85333879557559 and F1 43.8947152806227. The input/output validation passed;
full predictions have not been independently rescored locally in this update.
Compared with the rounded manuscript CAPP scores, the difference is about
1.56 EM / 1.80 F1 points in CAPP's favor, without a significance claim.
LambdaMART/monoT5 matched reader QA and full source-input comparability remain
separate outstanding items. See `notes/racs_manual_revisions_baselines_ablation.md`
for the completed-run record and ready-to-insert baseline/ablation revisions.
