...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize CAPP training-objective results.")
    parser.add_argument("--eval-json", required=True)
    parser.add_argument(
        "--model",
        action="append",
        required=True,
        help="Run and model path as OBJECTIVE:SEED=path/to/model.json.",
    )
...
def parse_model_spec(spec: str) -> tuple[str, int, Path]:
    if "=" not in spec or ":" not in spec.split("=", 1)[0]:
        raise ValueError(f"Expected OBJECTIVE:SEED=path, got {spec!r}")
    run, raw_path = spec.split("=", 1)
    objective, raw_seed = run.rsplit(":", 1)
    return objective, int(raw_seed), Path(raw_path)
...
def build_summary(
    ...
    for objective, seed, model_path in sorted(model_specs):
        label = f"{objective}_seed_{seed}"
...
def render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# CAPP Training-Objective Ablation",
        "",
        "Each run uses rank-stratified negative sampling, 50 negatives per labeled question, cap 20, the logistic scorer, and held-out-train page@4 alpha selection.",
        "",
        "| objective | seed | alpha | held-out train page@4 | dev page MRR | dev page@4 | dev doc@4 | recovered@4 | lost@4 | net@4 |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in summary["runs"]:
        lines.append(
            "| {objective} | {seed} | {selected_blend_alpha:.2f} | {heldout_train_page@4:.4f} | "
    ...
    lines.extend(
        [
            "",
            "| objective | alphas | held-out page@4 mean +/- std | dev page@4 mean +/- std | dev page MRR mean +/- std | dev page@10 mean +/- std |",
            "| --- | --- | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in summary["aggregate"]:
        ...
        lines.append(
            f"| {row['objective']} | {alphas} | {heldout['mean']:.4f} +/- {heldout['sample_std']:.4f} | "
