...
: "${DATA_NAME:?Set DATA_NAME, e.g. vidore-v3 or opendocvqa}"
...
: "${OUT_DIR:?Set OUT_DIR for cross-doc selection ablation outputs}"

SPLIT="${SPLIT:-dev}"
GOLD="${GOLD:-$DATA_ROOT/MMQA_${SPLIT}.jsonl}"
DOC_PAGES_JSONL="${DOC_PAGES_JSONL:-$DATA_ROOT/doc_pages_${SPLIT}.jsonl}"
LABEL_PREFIX="${LABEL_PREFIX:-${DATA_NAME}_crossdoc_select_ablation}"
...
run_variant() {
  ...
  local candidate_pool="$3"
  local new_doc_bonus="${4:-0.05}"
  local same_doc_penalty="${5:-0.05}"
  ...
  FINAL_SELECTION_CANDIDATE_POOL="$candidate_pool" \
  FINAL_SELECTION_NEW_DOC_BONUS="$new_doc_bonus" \
  FINAL_SELECTION_SAME_DOC_PENALTY="$same_doc_penalty" \
...
run_variant score_baseline score 20
run_variant max1doc_pool20 max1_per_doc_then_fill 20
run_variant max1doc_pool50 max1_per_doc_then_fill 50
run_variant mmr_docdiv_pool20_b0p02 mmr_doc_diverse 20 0.02 0.02
run_variant mmr_docdiv_pool20_b0p05 mmr_doc_diverse 20 0.05 0.05
run_variant mmr_docdiv_pool20_b0p10 mmr_doc_diverse 20 0.10 0.10
