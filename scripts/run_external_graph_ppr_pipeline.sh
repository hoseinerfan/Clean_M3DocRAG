...
QUERY_ANCHOR_CONSTRAINT_BUNDLE_WEIGHT="${QUERY_ANCHOR_CONSTRAINT_BUNDLE_WEIGHT:-1.0}"
QUERY_ANCHOR_CONSTRAINT_MIN_SLOT_TYPES="${QUERY_ANCHOR_CONSTRAINT_MIN_SLOT_TYPES:-2}"
QUERY_ANCHOR_CONSTRAINT_EXTRACTION_MODE="${QUERY_ANCHOR_CONSTRAINT_EXTRACTION_MODE:-legacy}"
QUERY_ANCHOR_CONSTRAINT_VALUE_WEIGHT_MODE="${QUERY_ANCHOR_CONSTRAINT_VALUE_WEIGHT_MODE:-legacy}"
QUERY_ANCHOR_CONSTRAINT_MAX_PAGE_MATCHES="${QUERY_ANCHOR_CONSTRAINT_MAX_PAGE_MATCHES:-0}"
QUERY_ANCHOR_CONSTRAINT_MAX_SLOT_PAGE_MATCHES="${QUERY_ANCHOR_CONSTRAINT_MAX_SLOT_PAGE_MATCHES:-0}"
QUERY_ANCHOR_CONSTRAINT_SPECIFICITY_FLOOR="${QUERY_ANCHOR_CONSTRAINT_SPECIFICITY_FLOOR:-0.10}"
QUERY_ANCHOR_CONSTRAINT_TABLE_BONUS="${QUERY_ANCHOR_CONSTRAINT_TABLE_BONUS:-0.0}"
QUERY_ANCHOR_CONSTRAINT_TABLE_MIN_SCORE="${QUERY_ANCHOR_CONSTRAINT_TABLE_MIN_SCORE:-0.0}"
HEADING_BREADCRUMB_MODE="${HEADING_BREADCRUMB_MODE:-none}"
HEADING_BREADCRUMB_FIELD="${HEADING_BREADCRUMB_FIELD:-markdown}"
HEADING_BREADCRUMB_EDGE_WEIGHT="${HEADING_BREADCRUMB_EDGE_WEIGHT:-0.10}"
HEADING_BREADCRUMB_RESTART_WEIGHT="${HEADING_BREADCRUMB_RESTART_WEIGHT:-0.10}"
HEADING_BREADCRUMB_MAX_HEADINGS_PER_PAGE="${HEADING_BREADCRUMB_MAX_HEADINGS_PER_PAGE:-8}"
HEADING_BREADCRUMB_MIN_TOKENS="${HEADING_BREADCRUMB_MIN_TOKENS:-1}"
HEADING_BREADCRUMB_MIN_TOKEN_LEN="${HEADING_BREADCRUMB_MIN_TOKEN_LEN:-3}"
HEADING_BREADCRUMB_QUERY_MIN_OVERLAP="${HEADING_BREADCRUMB_QUERY_MIN_OVERLAP:-0.60}"
HEADING_BREADCRUMB_MAX_PAGE_MATCHES="${HEADING_BREADCRUMB_MAX_PAGE_MATCHES:-80}"
HEADING_BREADCRUMB_MAX_DOC_MATCHES="${HEADING_BREADCRUMB_MAX_DOC_MATCHES:-20}"
HEADING_BREADCRUMB_WEIGHT_MODE="${HEADING_BREADCRUMB_WEIGHT_MODE:-local_idf}"
HEADING_BREADCRUMB_MIN_NODE_WEIGHT="${HEADING_BREADCRUMB_MIN_NODE_WEIGHT:-0.10}"
ENTITY_ALIAS_MODE="${ENTITY_ALIAS_MODE:-none}"
ENTITY_ALIAS_FIELD="${ENTITY_ALIAS_FIELD:-markdown}"
ENTITY_ALIAS_EDGE_WEIGHT="${ENTITY_ALIAS_EDGE_WEIGHT:-0.10}"
ENTITY_ALIAS_RESTART_WEIGHT="${ENTITY_ALIAS_RESTART_WEIGHT:-0.05}"
ENTITY_ALIAS_MAX_ENTITIES_PER_PAGE="${ENTITY_ALIAS_MAX_ENTITIES_PER_PAGE:-24}"
ENTITY_ALIAS_MIN_TOKEN_LEN="${ENTITY_ALIAS_MIN_TOKEN_LEN:-2}"
ENTITY_ALIAS_QUERY_MIN_OVERLAP="${ENTITY_ALIAS_QUERY_MIN_OVERLAP:-0.60}"
ENTITY_ALIAS_MAX_PAGE_MATCHES="${ENTITY_ALIAS_MAX_PAGE_MATCHES:-80}"
ENTITY_ALIAS_MAX_DOC_MATCHES="${ENTITY_ALIAS_MAX_DOC_MATCHES:-20}"
ENTITY_ALIAS_WEIGHT_MODE="${ENTITY_ALIAS_WEIGHT_MODE:-local_idf}"
ENTITY_ALIAS_MIN_NODE_WEIGHT="${ENTITY_ALIAS_MIN_NODE_WEIGHT:-0.10}"
CONSTRAINT_COMPETITION_MODE="${CONSTRAINT_COMPETITION_MODE:-none}"
CONSTRAINT_COMPETITION_STRENGTH="${CONSTRAINT_COMPETITION_STRENGTH:-0.0}"
QUERY_LOCAL_EVIDENCE_DOC_TOP_K="${QUERY_LOCAL_EVIDENCE_DOC_TOP_K:-20}"
QUERY_LOCAL_EVIDENCE_MIN_DOC_SUPPORT="${QUERY_LOCAL_EVIDENCE_MIN_DOC_SUPPORT:-0.0}"
QUERY_LOCAL_EVIDENCE_MAX_ATOMS="${QUERY_LOCAL_EVIDENCE_MAX_ATOMS:-24}"
QUERY_LOCAL_EVIDENCE_MAX_NGRAM="${QUERY_LOCAL_EVIDENCE_MAX_NGRAM:-4}"
QUERY_LOCAL_EVIDENCE_MIN_TOKEN_LEN="${QUERY_LOCAL_EVIDENCE_MIN_TOKEN_LEN:-3}"
QUERY_LOCAL_EVIDENCE_MIN_SPECIFICITY="${QUERY_LOCAL_EVIDENCE_MIN_SPECIFICITY:-0.0}"
QUERY_LOCAL_EVIDENCE_STOPWORD_MODE="${QUERY_LOCAL_EVIDENCE_STOPWORD_MODE:-generic}"
QUERY_LOCAL_EVIDENCE_CONFIDENCE_MODE="${QUERY_LOCAL_EVIDENCE_CONFIDENCE_MODE:-entropy}"
QUERY_LOCAL_EVIDENCE_MIN_DOC_CONFIDENCE="${QUERY_LOCAL_EVIDENCE_MIN_DOC_CONFIDENCE:-0.0}"
QUERY_LOCAL_EVIDENCE_MAX_ACTIVE_PAGE_FRAC="${QUERY_LOCAL_EVIDENCE_MAX_ACTIVE_PAGE_FRAC:-1.0}"
QUERY_LOCAL_EVIDENCE_SOFTMAX_MIX="${QUERY_LOCAL_EVIDENCE_SOFTMAX_MIX:-1.0}"
QUERY_LOCAL_EVIDENCE_SOFTMAX_PRIOR="${QUERY_LOCAL_EVIDENCE_SOFTMAX_PRIOR:-page_seed}"
QUERY_LOCAL_SELECTOR_FEATURE="${QUERY_LOCAL_SELECTOR_FEATURE:-mean_query_local_evidence_boundary_weight}"
QUERY_LOCAL_SELECTOR_DECISION_MODE="${QUERY_LOCAL_SELECTOR_DECISION_MODE:-threshold}"
QUERY_LOCAL_SELECTOR_OP="${QUERY_LOCAL_SELECTOR_OP:->=}"
QUERY_LOCAL_SELECTOR_THRESHOLD="${QUERY_LOCAL_SELECTOR_THRESHOLD:-0.0}"
QUERY_LOCAL_STABILITY_STRENGTH_FACTORS="${QUERY_LOCAL_STABILITY_STRENGTH_FACTORS:-0.5,0.75,1.0}"
QUERY_LOCAL_STABILITY_TOP_K="${QUERY_LOCAL_STABILITY_TOP_K:-4}"
QUERY_LOCAL_EVIDENCE_BOUNDARY_MODE="${QUERY_LOCAL_EVIDENCE_BOUNDARY_MODE:-none}"
QUERY_LOCAL_EVIDENCE_BOUNDARY_RANK_SOURCE="${QUERY_LOCAL_EVIDENCE_BOUNDARY_RANK_SOURCE:-page_seed}"
QUERY_LOCAL_EVIDENCE_BOUNDARY_CENTER_RANK="${QUERY_LOCAL_EVIDENCE_BOUNDARY_CENTER_RANK:-5.0}"
QUERY_LOCAL_EVIDENCE_BOUNDARY_WINDOW="${QUERY_LOCAL_EVIDENCE_BOUNDARY_WINDOW:-6.0}"
QUERY_LOCAL_EVIDENCE_BOUNDARY_FLOOR="${QUERY_LOCAL_EVIDENCE_BOUNDARY_FLOOR:-0.25}"
...
DOC_DOC_CONFIDENCE_MODE="${DOC_DOC_CONFIDENCE_MODE:-none}"
...
GRAPH_ARGS=(
  ...
  --query-anchor-constraint-bundle-weight "$QUERY_ANCHOR_CONSTRAINT_BUNDLE_WEIGHT"
  --query-anchor-constraint-min-slot-types "$QUERY_ANCHOR_CONSTRAINT_MIN_SLOT_TYPES"
  --query-anchor-constraint-extraction-mode "$QUERY_ANCHOR_CONSTRAINT_EXTRACTION_MODE"
  --query-anchor-constraint-value-weight-mode "$QUERY_ANCHOR_CONSTRAINT_VALUE_WEIGHT_MODE"
  --query-anchor-constraint-max-page-matches "$QUERY_ANCHOR_CONSTRAINT_MAX_PAGE_MATCHES"
  --query-anchor-constraint-max-slot-page-matches "$QUERY_ANCHOR_CONSTRAINT_MAX_SLOT_PAGE_MATCHES"
  --query-anchor-constraint-specificity-floor "$QUERY_ANCHOR_CONSTRAINT_SPECIFICITY_FLOOR"
  --query-anchor-constraint-table-bonus "$QUERY_ANCHOR_CONSTRAINT_TABLE_BONUS"
  --query-anchor-constraint-table-min-score "$QUERY_ANCHOR_CONSTRAINT_TABLE_MIN_SCORE"
  --heading-breadcrumb-mode "$HEADING_BREADCRUMB_MODE"
  --heading-breadcrumb-edge-weight "$HEADING_BREADCRUMB_EDGE_WEIGHT"
  --heading-breadcrumb-restart-weight "$HEADING_BREADCRUMB_RESTART_WEIGHT"
  --heading-breadcrumb-max-headings-per-page "$HEADING_BREADCRUMB_MAX_HEADINGS_PER_PAGE"
  --heading-breadcrumb-min-tokens "$HEADING_BREADCRUMB_MIN_TOKENS"
  --heading-breadcrumb-min-token-len "$HEADING_BREADCRUMB_MIN_TOKEN_LEN"
  --heading-breadcrumb-query-min-overlap "$HEADING_BREADCRUMB_QUERY_MIN_OVERLAP"
  --heading-breadcrumb-max-page-matches "$HEADING_BREADCRUMB_MAX_PAGE_MATCHES"
  --heading-breadcrumb-max-doc-matches "$HEADING_BREADCRUMB_MAX_DOC_MATCHES"
  --heading-breadcrumb-weight-mode "$HEADING_BREADCRUMB_WEIGHT_MODE"
  --heading-breadcrumb-min-node-weight "$HEADING_BREADCRUMB_MIN_NODE_WEIGHT"
  --entity-alias-mode "$ENTITY_ALIAS_MODE"
  --entity-alias-edge-weight "$ENTITY_ALIAS_EDGE_WEIGHT"
  --entity-alias-restart-weight "$ENTITY_ALIAS_RESTART_WEIGHT"
  --entity-alias-max-entities-per-page "$ENTITY_ALIAS_MAX_ENTITIES_PER_PAGE"
  --entity-alias-min-token-len "$ENTITY_ALIAS_MIN_TOKEN_LEN"
  --entity-alias-query-min-overlap "$ENTITY_ALIAS_QUERY_MIN_OVERLAP"
  --entity-alias-max-page-matches "$ENTITY_ALIAS_MAX_PAGE_MATCHES"
  --entity-alias-max-doc-matches "$ENTITY_ALIAS_MAX_DOC_MATCHES"
  --entity-alias-weight-mode "$ENTITY_ALIAS_WEIGHT_MODE"
  --entity-alias-min-node-weight "$ENTITY_ALIAS_MIN_NODE_WEIGHT"
  --constraint-competition-mode "$CONSTRAINT_COMPETITION_MODE"
  --constraint-competition-strength "$CONSTRAINT_COMPETITION_STRENGTH"
  --query-local-evidence-doc-top-k "$QUERY_LOCAL_EVIDENCE_DOC_TOP_K"
  --query-local-evidence-min-doc-support "$QUERY_LOCAL_EVIDENCE_MIN_DOC_SUPPORT"
  --query-local-evidence-max-atoms "$QUERY_LOCAL_EVIDENCE_MAX_ATOMS"
  --query-local-evidence-max-ngram "$QUERY_LOCAL_EVIDENCE_MAX_NGRAM"
  --query-local-evidence-min-token-len "$QUERY_LOCAL_EVIDENCE_MIN_TOKEN_LEN"
  --query-local-evidence-min-specificity "$QUERY_LOCAL_EVIDENCE_MIN_SPECIFICITY"
  --query-local-evidence-stopword-mode "$QUERY_LOCAL_EVIDENCE_STOPWORD_MODE"
  --query-local-evidence-confidence-mode "$QUERY_LOCAL_EVIDENCE_CONFIDENCE_MODE"
  --query-local-evidence-min-doc-confidence "$QUERY_LOCAL_EVIDENCE_MIN_DOC_CONFIDENCE"
  --query-local-evidence-max-active-page-frac "$QUERY_LOCAL_EVIDENCE_MAX_ACTIVE_PAGE_FRAC"
  --query-local-evidence-softmax-mix "$QUERY_LOCAL_EVIDENCE_SOFTMAX_MIX"
  --query-local-evidence-softmax-prior "$QUERY_LOCAL_EVIDENCE_SOFTMAX_PRIOR"
  --query-local-selector-feature "$QUERY_LOCAL_SELECTOR_FEATURE"
  --query-local-selector-decision-mode "$QUERY_LOCAL_SELECTOR_DECISION_MODE"
  --query-local-selector-op "$QUERY_LOCAL_SELECTOR_OP"
  --query-local-selector-threshold "$QUERY_LOCAL_SELECTOR_THRESHOLD"
  --query-local-stability-strength-factors "$QUERY_LOCAL_STABILITY_STRENGTH_FACTORS"
  --query-local-stability-top-k "$QUERY_LOCAL_STABILITY_TOP_K"
  --query-local-evidence-boundary-mode "$QUERY_LOCAL_EVIDENCE_BOUNDARY_MODE"
  --query-local-evidence-boundary-rank-source "$QUERY_LOCAL_EVIDENCE_BOUNDARY_RANK_SOURCE"
  --query-local-evidence-boundary-center-rank "$QUERY_LOCAL_EVIDENCE_BOUNDARY_CENTER_RANK"
  --query-local-evidence-boundary-window "$QUERY_LOCAL_EVIDENCE_BOUNDARY_WINDOW"
  --query-local-evidence-boundary-floor "$QUERY_LOCAL_EVIDENCE_BOUNDARY_FLOOR"
  ...
  --doc-doc-embedding-mutual-top-k "$DOC_DOC_EMBEDDING_MUTUAL_TOP_K"
  --doc-doc-rescue-anchor-top-k "$DOC_DOC_RESCUE_ANCHOR_TOP_K"
  --doc-doc-rescue-rank-min "$DOC_DOC_RESCUE_RANK_MIN"
  --doc-doc-rescue-rank-max "$DOC_DOC_RESCUE_RANK_MAX"
  --doc-doc-confidence-mode "$DOC_DOC_CONFIDENCE_MODE"
  --doc-doc-confidence-margin "$DOC_DOC_CONFIDENCE_MARGIN"
...
read -r -a HEADING_BREADCRUMB_FIELD_ARRAY <<< "$HEADING_BREADCRUMB_FIELD"
if [[ "${#HEADING_BREADCRUMB_FIELD_ARRAY[@]}" -gt 0 ]]; then
  GRAPH_ARGS+=(--heading-breadcrumb-field "${HEADING_BREADCRUMB_FIELD_ARRAY[@]}")
fi
read -r -a ENTITY_ALIAS_FIELD_ARRAY <<< "$ENTITY_ALIAS_FIELD"
if [[ "${#ENTITY_ALIAS_FIELD_ARRAY[@]}" -gt 0 ]]; then
  GRAPH_ARGS+=(--entity-alias-field "${ENTITY_ALIAS_FIELD_ARRAY[@]}")
fi
