...
class ApplyTrainedContentAwarePageRerankerTests(unittest.TestCase):
    ...
    def test_applies_saved_base_aware_alpha_utility_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            model_path = root / "model.json"
            pred_path = root / "pred.json"
                ...
                json.dumps(
                    {
                        ...
                        "adaptive_alpha_config": {
                            ...
                            "query_alpha_feature_top_k": 2,
                            "alpha_grid": [0.0, 1.0],
                            "recovery_weights": [0.0] * utility_feature_len,
                            "recovery_bias": 0.1,
                            "recovery_feature_mean": [0.0] * utility_feature_len,
                            "recovery_feature_std": [1.0] * utility_feature_len,
                            "loss_weights": [0.0] * utility_feature_len,
                            "loss_bias": 1.0,
                            "loss_feature_mean": [0.0] * utility_feature_len,
                            "loss_feature_std": [1.0] * utility_feature_len,
                        ...
                        "args": {
                            ...
                            "inference_mode": "blend_rerank",
                            "blend_alpha": 1.0,
                            "base_aware_alpha_utility_gate": True,
                        },
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(output["q1"]["page_retrieval_results"][0][:2], ["docA", 0])
            metadata = output["q1"]["trained_content_aware_transfer_metadata"]
            self.assertEqual(metadata["query_alpha_mode"], "base_aware_alpha_utility_gate")
            self.assertTrue(metadata["base_aware_alpha_utility_gate"])
            self.assertEqual(metadata["blend_alpha"], 0.0)
