...
OUT_DIR="${OUT_DIR:-$REPO_ROOT/output/m3docvqa_ltr_page_reranker}"
LABEL="${LABEL:-mmqa_train_to_dev_graph_aware_ltr_gpp_no_hyperlink}"
...
TRAIN_DENSE_PRED="${TRAIN_DENSE_PRED:-$GPP_TRAIN_OUT_DIR/${GPP_TRAIN_LABEL_PREFIX}_no_hyperlink.prediction.json}"
EVAL_DENSE_PRED="${EVAL_DENSE_PRED:-$GPP_EVAL_OUT_DIR/${GPP_EVAL_LABEL_PREFIX}_no_hyperlink.prediction.json}"
...
require_file train_base_pred "$TRAIN_DENSE_PRED"
require_file eval_base_pred "$EVAL_DENSE_PRED"
...
auto_tune_args=()
if [[ "${AUTO_TUNE_BLEND_ALPHA:-1}" == "1" ]]; then
...
echo "using_train_base_pred=$TRAIN_DENSE_PRED"
echo "using_eval_base_pred=$EVAL_DENSE_PRED"
echo "using_out_dir=$OUT_DIR"
echo "using_label=$LABEL"
echo "using_ltr_backend=${LTR_BACKEND:-auto}"
