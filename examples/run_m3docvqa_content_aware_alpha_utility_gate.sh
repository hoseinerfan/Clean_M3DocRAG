...
OUT_DIR="${OUT_DIR:-$REPO_ROOT/output/m3docvqa_content_aware_alpha_utility_gate_page5}"
LABEL="${LABEL:-mmqa_train_to_dev_content_aware_alpha_utility_gate_page5}"
...
echo
echo "== M3DocVQA content-aware alpha utility gate =="
...
echo
echo "== Alpha utility gate recall table =="
run_args=(
  --run "gpp_no_hyperlink=$EVAL_BASE_PRED"
  --run "alpha_utility_gate=$PRED_PATH"
)
...
if [[ "${RUN_TRANSFER:-1}" == "1" ]]; then
  echo
  echo "== Zero-shot transfer: alpha utility gate =="
  ...
  OUTPUT_SUBDIR="${OUTPUT_SUBDIR:-trained_content_aware_transfer_alpha_utility_gate}" \
