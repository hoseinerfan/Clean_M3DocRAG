...
OUT_DIR="${OUT_DIR:-$REPO_ROOT/output/m3docvqa_content_aware_adaptive_topk_ablation}"
...
AUTO_TOP_KS="${AUTO_TOP_KS:-4 5 10}"
FIXED_ALPHAS="${FIXED_ALPHAS:-0.20 0.30 0.40}"
...
run_variant() {
  ...
  local tune_hit_k="$3"
  local blend_alpha="$4"
  local label="mmqa_train_to_dev_content_aware_${label_suffix}_base_${BASE_LABEL}"
  local pred="$OUT_DIR/${label}.dev.prediction.json"

  if [[ "$FORCE_RERUN" != "1" && -f "$pred" ]]; then
    echo "reuse_${label_suffix}=$pred"
  else
    echo
    echo "== M3DocVQA content-aware top-k ablation: $label_suffix =="
    ...
    AUTO_TUNE_BLEND_ALPHA="$auto_tune" \
  ...
  add_eval_run "$label_suffix" "$pred"
}
...
for alpha in $FIXED_ALPHAS; do
  run_variant "fixed_alpha_$(tag_value "$alpha")" 0 5 "$alpha"
done

for hit_k in $AUTO_TOP_KS; do
  run_variant "auto_page${hit_k}" 1 "$hit_k" 0.30
done
...
"$PYTHON_BIN" "$REPO_ROOT/scripts/evaluate_pseudo_page_retrieval.py" \
  ...
  "${eval_args[@]}" \
  --format markdown \
  --output "$OUT_DIR/adaptive_topk_ablation_eval.md"
"$PYTHON_BIN" "$REPO_ROOT/scripts/evaluate_pseudo_page_retrieval.py" \
  --gold "$EVAL_GOLD" \
  "${eval_args[@]}" \
  --format csv \
  --output "$OUT_DIR/adaptive_topk_ablation_eval.csv"

echo "saved_eval_md=$OUT_DIR/adaptive_topk_ablation_eval.md"
echo "saved_eval_csv=$OUT_DIR/adaptive_topk_ablation_eval.csv"
