#!/bin/bash -l
#SBATCH --job-name=racs-controlled-qa
...
#SBATCH --output=output/racs_controlled_reader_%j.out
#SBATCH --error=output/racs_controlled_reader_%j.err
...
RACS_READER_DIR="$PWD/output/racs_controlled_reader_top4_${SLURM_JOB_ID:?}"
RACS_CHECK="$PWD/scripts/validate_racs_controlled_reader.py"
...
mkdir "$RACS_READER_DIR"
printf 'CUDA_VISIBLE_DEVICES=%s\n' "${CUDA_VISIBLE_DEVICES:-unset}"
if command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi --query-gpu=index,name,memory.total,driver_version --format=csv,noheader || true
fi

env/bin/python -B "$RACS_CHECK" prepare --repo-root "$PWD" --run-dir "$RACS_READER_DIR"
...
env/bin/python -B "$RACS_CHECK" check --repo-root "$PWD" --run-dir "$RACS_READER_DIR"
printf 'saved_controlled_reader_directory=%s\n' "$RACS_READER_DIR"
