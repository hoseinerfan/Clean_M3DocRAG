...
MODEL_NAME_OR_PATH="${MODEL_NAME_OR_PATH:-BAAI/bge-reranker-base}"
MODEL_TAG="${MODEL_TAG:-bge_reranker_base}"
RERANK_TOP_K="${RERANK_TOP_K:-100}"
LABEL="${LABEL:-mmqa_dev_${MODEL_TAG}_cross_encoder_gpp_top${RERANK_TOP_K}}"
...
"$PYTHON_BIN" "$REPO_ROOT/scripts/rerank_m3docvqa_cross_encoder_pages.py" \
  --gold "$EVAL_GOLD" \
  ...
  --max-page-chars "${MAX_PAGE_CHARS:-6000}" \
