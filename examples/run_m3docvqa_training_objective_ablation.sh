...
OUT_DIR="${OUT_DIR:-$REPO_ROOT/output/m3docvqa_content_aware_exact_maxsim_training_objective_ablation}"
...
OBJECTIVES="${OBJECTIVES:-weighted_bce pairwise_ranknet}"
...
label_for_run() {
  printf 'mmqa_train_to_dev_content_aware_objective_%s_seed_%s_poscap20_auto_page4_base_%s' "$1" "$2" "$BASE_LABEL"
}
...
prediction_for_run() {
  ...
  if [[ "$REUSE_SEED_STABILITY" == "1" && "$objective" == "weighted_bce" && -f "$reference" ]]; then
    printf '%s' "$reference"
  else
    printf '%s/%s.dev.prediction.json' "$OUT_DIR" "$(label_for_run "$objective" "$seed")"
  fi
}
...
model_for_run() {
  ...
  if [[ "$REUSE_SEED_STABILITY" == "1" && "$objective" == "weighted_bce" && -f "$reference" ]]; then
    ...
    printf '%s/%s.model.json' "$OUT_DIR" "$(label_for_run "$objective" "$seed")"
  fi
}
...
run_one() {
  ...
  label="$(label_for_run "$objective" "$seed")"
  pred="$(prediction_for_run "$objective" "$seed")"
  model="$(model_for_run "$objective" "$seed")"
  if [[ "$FORCE_RERUN" != "1" && -f "$pred" && -f "$model" ]]; then
    echo "reuse_${objective}_seed_${seed}=$pred"
    return 0
  fi

  echo
  echo "== CAPP training objective: $objective, seed $seed =="
  ...
  NEGATIVE_SAMPLING_STRATEGY=rank_stratified \
...
if [[ "$RUN_EVAL" == "1" ]]; then
  ...
  for objective in $OBJECTIVES; do
    for seed in $SEEDS; do
      pred="$(prediction_for_run "$objective" "$seed")"
      model="$(model_for_run "$objective" "$seed")"
      require_file "${objective}_seed_${seed}_prediction" "$pred"
      require_file "${objective}_seed_${seed}_model" "$model"
      eval_args+=(--run "${objective}_seed_${seed}=$pred")
      model_args+=(--model "${objective}:${seed}=$model")
    done
  done
  ...
  "$PYTHON_BIN" "$REPO_ROOT/scripts/evaluate_pseudo_page_retrieval.py" \
    ...
    "${eval_args[@]}" \
    --format markdown \
    --output "$OUT_DIR/training_objective_runs.md"
  ...
  "$PYTHON_BIN" "$REPO_ROOT/scripts/evaluate_pseudo_page_retrieval.py" \
    ...
    "${eval_args[@]}" \
    --format json \
    --output "$OUT_DIR/training_objective_runs.json"

  "$PYTHON_BIN" "$REPO_ROOT/scripts/summarize_m3docvqa_training_objective_ablation.py" \
    --eval-json "$OUT_DIR/training_objective_runs.json" \
    "${model_args[@]}" \
    --output-md "$OUT_DIR/training_objective_summary.md" \
    --output-json "$OUT_DIR/training_objective_summary.json"
fi
