...
REPORT_OUT="${REPORT_OUT:-$REPO_ROOT/zero_shot_content_aware_results.md}"
REPORT_CSV_OUT="${REPORT_CSV_OUT:-$REPO_ROOT/zero_shot_content_aware_results.csv}"
ZERO_SHOT_OUTPUT_SUBDIR="${ZERO_SHOT_OUTPUT_SUBDIR:-zero_shot_content_aware}"

CANDIDATE_TOP_K="${CANDIDATE_TOP_K:-1000}"
INFERENCE_MODE="${INFERENCE_MODE:-blend_rerank}"
BLEND_ALPHA="${BLEND_ALPHA:-0.20}"
PROTECT_TOP_K="${PROTECT_TOP_K:-1}"
PROMOTION_RANK_MIN="${PROMOTION_RANK_MIN:-5}"
PROMOTION_RANK_MAX="${PROMOTION_RANK_MAX:-100}"
MAX_PROMOTIONS_PER_QID="${MAX_PROMOTIONS_PER_QID:-2}"
MIN_CONTENT_SCORE="${MIN_CONTENT_SCORE:-0.35}"
SOURCE_BONUS="${SOURCE_BONUS:-0.08}"
FIRST_PAGE_PENALTY="${FIRST_PAGE_PENALTY:-0.00}"
...
run_variant() {
  ...
  echo
  echo "== $display_name zero-shot content-aware: $label =="
  source_args=()
  ...
  "$PYTHON_BIN" "$REPO_ROOT/scripts/zero_shot_content_aware_page_reranker.py" \
    --base-pred "$base_pred" \
...
run_dataset() {
  ...
  alpha="$(alpha_tag "$BLEND_ALPHA")"
  out_dir="$work_root/output/$output_slug/$ZERO_SHOT_OUTPUT_SUBDIR"
  mkdir -p "$out_dir"
  ...
  if [[ "$RUN_DENSE_BASE" == "1" ]]; then
    run_variant "$display_name" "$data_name" "$gold" "$doc_pages" "$dense_pred" "$sparse_pred" \
      "$out_dir" "${data_name}_zero_shot_dense_${INFERENCE_MODE}_a${alpha}"
  fi
  ...
  if [[ "$RUN_DOCSEED_BASE" == "1" ]]; then
    ...
    run_variant "$display_name" "$data_name" "$gold" "$doc_pages" "$docseed_pred" "$sparse_pred" \
      "$out_dir" "${data_name}_zero_shot_docseed_rrf_1p00_${INFERENCE_MODE}_a${alpha}"
  fi
}
